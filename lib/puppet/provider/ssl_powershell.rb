...
class Puppet::Provider::SSL_PowerShell < Puppet::Provider # rubocop:disable all
  require Pathname.new(__FILE__).dirname + '..' + '..' + 'puppet_x' + 'puppetlabs' + 'ssliis' + 'powershell_manager'
  require Pathname.new(__FILE__).dirname + '..' + '..' + 'puppet_x' + 'puppetlabs' + 'ssliis' + 'powershell_common'
  include PuppetX::SSLIIS::PowerShellCommon
  ...
  def self.ps_manager
    PuppetX::SSLIIS::PowerShellManager.instance("#{command(:powershell)} #{PuppetX::SSLIIS::PowerShellCommon.powershell_args.join(' ')}")
  end
  ...
  def self.ps_script_content(template, resource)
    ...
    template_file = File.new(template_path + "/letsencrypt/#{template}.ps1.erb").read
